...
describe('DayPicker', () => {
  ...
  describe('#onKeyDown', () => {
    describe('focusedDate is truthy', () => {
      ...
      describe('?', () => {
        it('calls toggleKeyboardShortcuts', () => {
          const toggleKeyboardShortcutsSpy = sinon.spy(DayPicker.prototype, 'toggleKeyboardShortcuts');
          const wrapper = shallow(<DayPicker />);
          wrapper.setState({
            focusedDate: today,
          });
          wrapper.instance().onKeyDown({ ...event, key: '?' });
          expect(toggleKeyboardShortcutsSpy.callCount).to.equal(1);
        });
      });
  ...
  describe('#toggleKeyboardShortcuts', () => {
    it('sets state.showKeyboardShortcuts to true if false', () => {
      const wrapper = shallow(<DayPicker />);
      wrapper.setState({
        showKeyboardShortcuts: false,
      });
      wrapper.instance().toggleKeyboardShortcuts();
      expect(wrapper.state().showKeyboardShortcuts).to.equal(true);
    });

    it('sets state.showKeyboardShortcuts to false if true', () => {
      const wrapper = shallow(<DayPicker />);
      wrapper.setState({
        showKeyboardShortcuts: true,
      });
